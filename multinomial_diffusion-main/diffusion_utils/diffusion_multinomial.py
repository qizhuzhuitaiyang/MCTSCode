...
class MultinomialDiffusion(torch.nn.Module):
    ...
    def predict_start(self, log_x_t, t, context=None):
        """
        Predict x_0 from x_t with optional condition
        
        Args:
            log_x_t: log one-hot encoded x_t
            t: timestep
            context: optional condition embedding (B, condition_dim)
        """
        x_t = log_onehot_to_index(log_x_t)

        out = self._denoise_fn(t, x_t, context=context)
    ...
    def p_pred(self, log_x, t, context=None):
        """Model prediction with optional condition"""
        if self.parametrization == 'x0':
            log_x_recon = self.predict_start(log_x, t=t, context=context)
            log_model_pred = self.q_posterior(
                log_x_start=log_x_recon, log_x_t=log_x, t=t)
        elif self.parametrization == 'direct':
            log_model_pred = self.predict_start(log_x, t=t, context=context)
        else:
            raise ValueError
        return log_model_pred

    @torch.no_grad()
    def p_sample(self, log_x, t, context=None):
        """Single denoising step with optional condition"""
    ...
    def p_sample_loop(self, shape, context=None):
        """
        Full sampling loop with optional condition
        ...
        Returns:
            Sampled output (indices)
        """
        device = self.log_alpha.device
        b = shape[0]
        
        # Initialize with uniform categorical distribution (not Gaussian!)
        uniform_logits = torch.zeros((b, self.num_classes) + self.shape, device=device)
        log_z = self.log_sample_categorical(uniform_logits)
        
        # Reverse diffusion process from T-1 to 0
    ...
    @torch.no_grad()
    def _sample(self, image_size, batch_size = 16, context=None):
        """Sample with optional condition (for image-like data)"""
        return self.p_sample_loop((batch_size, 3, image_size, image_size), context=context)

    @torch.no_grad()
    def interpolate(self, x1, x2, t = None, lam = 0.5, context=None):
        """Interpolation between two samples with optional condition"""
        ...
        for i in reversed(range(0, t)):
            img = self.p_sample(img, torch.full((b,), i, device=device, dtype=torch.long), context=context)
    ...
    def compute_Lt(self, log_x_start, log_x_t, t, context=None, detach_mean=False):
        """
        Compute loss term L_t with optional condition
        
        Args:
            log_x_start: log one-hot encoded x_0
            log_x_t: log one-hot encoded x_t
            t: timestep
            context: optional condition embedding (B, condition_dim)
            detach_mean: whether to detach mean
        """
    ...
    def _train_loss(self, x, context=None):
        """
        Training loss with optional condition
        ...
        if self.loss_type == 'vb_stochastic':
            ...
            kl = self.compute_Lt(
                log_x_start, self.q_sample(log_x_start=log_x_start, t=t), t, context=context)
    ...
    def log_prob(self, x, context=None):
        """
        Compute log probability with optional condition
        ...
        Returns:
            Log probability (negative loss)
        """
        b, device = x.size(0), x.device
        if self.training:
            return self._train_loss(x, context=context)
    ...
    def sample(self, num_samples, context=None):
        """
        Unconditional or conditional sampling
    ...
    def sample_chain(self, num_samples, context=None):
        """
        Sample full denoising chain with optional condition
