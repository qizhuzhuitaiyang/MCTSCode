...
def get_model(args, data_shape, num_classes):
    ...
    # Get condition dimension for conditional generation (e.g., patient embedding)
    condition_dim = getattr(args, 'condition_dim', None)
    ...
    class DynamicsTransformer(nn.Module):
        def __init__(self):
            ...
            self.transformer = LinearAttentionTransformerEmbedding(
                ...
                condition_dim=condition_dim,  # condition embedding dimension (e.g., 1024 for patient info)
            )
        ...
        def forward(self, t, x, context=None):
            ...
            Returns:
                Output logits (B, num_classes, L)
            """
            x = self.transformer(x, t, context=context)
            x = x.permute(0, 2, 1)
            x = self.rezero(x)
            return x
