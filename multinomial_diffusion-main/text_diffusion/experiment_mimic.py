...
class MIMICExperiment(DiffusionExperiment):
    ...
    def train_fn(self, epoch):
        ...
        for iteration, batch in enumerate(self.train_loader):
            # Unpack 3 elements: drug_indices, condition_embedding, length
            x, context, length = batch
            x = x.to(self.args.device)
            context = context.to(self.args.device)
            length = length.to(self.args.device)
            
            # x is already drug indices (0 or 1 for each drug)
            num_elem = length.sum()
            # Pass context to log_prob for conditional training
            loss = - self.model.log_prob(x, context=context).sum() / (math.log(2) * num_elem)
            loss.backward()
    ...
    def eval_fn(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch in self.eval_loader:
                ...
                length = length.to(self.args.device)
                num_elem = length.sum()
                # Pass context for conditional evaluation
                loss = - self.model.log_prob(x, context=context).sum() / (math.log(2) * num_elem)
                loss_sum += loss.detach().cpu().item() * len(x)
                loss_count += len(x)
